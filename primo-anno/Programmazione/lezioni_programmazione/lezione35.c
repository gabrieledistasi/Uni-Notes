...
La definizione aggiornata della struttura è contenuta del file dizionario.h che deve essere incluso in questo modo*/
...
k deve essere una chiave del dizionario, deve essere preventivamente verificato con in_dizionario.*/
...
Il programma legge ripetutamente stringhe dallo standard input (stdin) e le utilizza come chiavi di un dizionario.

A ciascuna chiave è associato un valore intero che rappresenta il numero di occorrenze della stringa letta.

L'inserimento e l'aggiornamento delle chiavi avviene dinamicamente, utilizzando una struttura dati "dizionario".*/
...
/*Se l'elenco di parole è contenuto in un file, ad esempio Moby Dick, è possibile fornire il file come input al programma tramite la redirezione dello standard input.
...
Compilazione
Il programma è diviso i tre file: de definizioni dei tipi di dato sono sul file dizionario.h e le funzioni delle liste sono definite sul file lista.c. Per compilare il programma occorre compilare insieme il presente file con il file lista.c.
